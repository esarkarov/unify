          ...
          <Popover
            ...
            <PopoverTrigger asChild>
              <Button
                ...
                className={cn('w-full', 'min-w-48', 'max-w-80', 'justify-start', 'h-auto', 'min-h-9')}>
                      ...
                      {getSelectedLabels()
                        ...
                        .map(({ label, value: val }) => (
                          <Badge
                            key={val}
                            variant="outline"
                            className={cn('inline-flex', 'items-center', 'gap-0', 'h-4', 'pr-0.5', 'rounded-sm')}>
...
export function DataTableFilterDropdownDateSinglePicker<TData>({
  ...
  const [filterValue, setFilterValue] = useState<Date | undefined>(() => parseDate(columnFilterValue));
...
export function DataTableFilterDropdownDateRangePicker<TData>({
  ...
  const [filterValue, setFilterValue] = useState<DateRange | undefined>(() => parseDateRange(columnFilterValue));
  ...
  const handleApply = () => {
    ...
    const values = formatDateRange?.(filterValue) ?? [filterValue.from.toISOString(), filterValue.to.toISOString()];
...
export type DataTableFilterInputProps<TData> = {
  ...
  defaultOperator?: CrudOperators;
  operators?: CrudOperators[];
  renderInput: (props: { value: string | string[]; onChange: (value: string | string[]) => void }) => React.ReactNode;
};
...
export function DataTableFilterInput<TData>({
  ...
  const [filterValue, setFilterValue] = useState((columnFromProps.getFilterValue() as string | string[]) || '');
